...
import { Award, MessageSquare, RefreshCw, Sparkles } from "lucide-react";
...
export default function Communication() {
  ...
  const handleStartExam = async () => {
    setState("Loading");
    try {
      const q = await fetchAIQuestions("Communication", user?.selectedRole || "Frontend Developer", difficulty);
      setQuestions(q);
      setCurrentIndex(0);
      setAnswers({});
      setIsLocked(false);
      ...
      setState("Config");
      alert("Error occurred configuring Communication skill practice sets.");
    }
  };
  ...
  const handleNext = () => {
    if (currentIndex < questions.length - 1) {
      ...
      history.unshift({
        id: "sess_comm_" + Math.random().toString(36).substring(2, 6),
      ...
      if (progressStr) {
        try {
          ...
          m.categoryProgress["Communication"] = Math.min(100, (m.categoryProgress["Communication"] || 10) + 15);
          m.streak += 1;
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        <h2 className="text-xl font-bold text-slate-800">Behavioral & Business Communication Practice</h2>
        <p className="text-xs text-slate-500">Master client communications, technical negotiation, conflict structures, and active listening.</p>
      </div>

      {state === "Config" && (
        <div className="bg-white border border-slate-200 p-6 rounded-2xl max-w-xl space-y-4 shadow-xs">
          <h3 className="text-sm font-bold text-slate-800 uppercase">Communication Session Settings</h3>
          <div className="space-y-3">
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-slate-500 uppercase">Select Target Scenario Difficulty</label>
              <select
                value={difficulty}
              ...
              >
                <option value="Easy">Easy (Everyday corporate alignments & general STAR outline)</option>
                <option value="Medium">Medium (Complex client scope creep & agile task allocations)</option>
                <option value="Hard">Hard (Expert stakeholder conflict resolutions & trade-off designs)</option>
              </select>
            </div>
            ...
            <div className="p-3.5 bg-indigo-50/50 border border-indigo-100 rounded-xl text-[11px] text-slate-600 flex items-start gap-2">
              ...
              <span>STAR methodologies evaluate Situation, Task, Action, and Result formats directly to build strong soft-skills.</span>
            </div>
          </div>
          <button 
      ...
      {state === "Loading" && (
        <div className="bg-white border border-slate-200 p-10 rounded-2xl flex flex-col justify-center items-center text-center space-y-3 min-h-68">
          ...
          <p className="text-xs font-bold text-slate-700 uppercase">Structuring Business scenarios...</p>
        </div>
      )}
      ...
      {state === "Summary" && (
        <div className="bg-white border border-slate-205 p-8 rounded-2xl text-center max-w-md space-y-4 shadow-sm">
          ...
          <div>
            <h3 className="text-base font-black text-slate-800 leading-snug">Communication Set cleared!</h3>
            <p className="text-xs text-slate-400 mt-1">Evaluation logs updated in user history folders. Let's inspect accuracy:</p>
          </div>
